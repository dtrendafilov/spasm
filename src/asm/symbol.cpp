...
namespace SpasmImpl
{
	namespace ASM
	{
		const size_t Symbol::notdefined = ~0;

		Symbol::Symbol()
			: _definition (notdefined)
		{ }
		...
		*/
		Symbol * Symbol_Table::insert (const std::string &identifier,
				size_t position)
		{
		...
		** \return pointer to the symbol
		*/
		Symbol * Symbol_Table::define (const std::string &identifier,
				size_t definition)
		{
			Symbol *psymbol = find (identifier); 
			if (psymbol == NULL) {
